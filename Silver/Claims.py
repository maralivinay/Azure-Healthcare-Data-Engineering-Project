...
spark.conf.set(
  "fs.azure.account.key.storagevinaydataproject.dfs.core.windows.net",
  dbutils.secrets.get(scope="tt-hc-kv", key="storageAC")
)
...
claims_df=spark.read.csv("abfss://landing@storagevinaydataproject.dfs.core.windows.net/Claims/*.csv",header=True)

claims_df = claims_df.withColumn(
    "datasource",
    f.when(f.col("_metadata.file_path").contains("hospital1"), "hosa").when(f.col("_metadata.file_path").contains("hospital2"), "hosb")
     .otherwise(None)
)
...
claims_df.write.format("parquet") \
    .mode("overwrite") \
    .save("abfss://bronze@storagevinaydataproject.dfs.core.windows.net/Claims/")
